...
class foundPage extends BaseClass {
  public ele: any;
  public state = {
    noBto: false
  }
  ...
  render() {
    return (
      <div className="foundPage anmateContent">
        {/* <NavBar btnGoback={true} btnService={true} title={'发现'}></NavBar> */}
        <div className='pageContent' style={{ height: this.state.noBto ? "100%" : "calc(100% - 1rem)" }}>
          <iframe title='liveShow' src={`https://tyt365.com/nav/#${window.location}||${this.props.user.ExtToken}`} ></iframe>
        </div>
      </div>
    );
  }
